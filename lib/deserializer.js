...
function processResponse(desc, response) {
    ...
    for (var childName in response) {
        if (soaputils.isElement(childName)) {
            ...
            if (part) {
                var partName = part.jsName ? part.jsName : part.name;
                processType(result, partName, child, part.jsonType, part.type, desc.isEncoded, part.isArray);
            }
        }
    }
...
function processObject(source, type, isEncoded) {
    ...
    function descIsArray(desc) {
        return desc.isArray || desc.maxOccurs < 0 || desc.maxOccurs > 1;
    }
    ...
    for (var childName in source) {
        if (soaputils.isElement(childName)) {
            ...
            var ns = childIsArray ? child[0]['@ns'] : child['@ns'];
            var desc = type.hash[soaputils.makeQnameFromXml(ns)];
            if (!desc) {
                if (childIsArray) {
                    for (var descName in type.hash) {
                        var candidate = type.hash[descName];
                        if (descIsArray(candidate)) {
                            desc = candidate;
                            break;
                        }
                    }
            ...
            if (desc) {
                var isArray = descIsArray(desc);
                if (desc && !desc.isAttr) {
                    var descName = desc.jsName ? desc.jsName : desc.name;
                    processType(result, descName, child, desc.jsonType, desc.type, isEncoded, isArray);
                }
            }
        }
    }
        ...
        for (var attrName in attrs) {
            ...
            switch(attr.uri)  {
                ...
                default:
                    ...
                    if (desc && desc.isAttr) {
                        var descName = desc.jsName ? desc.jsName : desc.name;
                        processType(result, descName, result, desc.jsonType, null, isEncoded, false);
                    }
            }
        }
    }
...
function makePartsHash(desc) {
    ...
    desc.parts.forEach(function(part) {
        if (part.elementName) {
            ...
            desc.hash[soaputils.makeQname(part.ns, part.name)] = part;
        }
    });
}
