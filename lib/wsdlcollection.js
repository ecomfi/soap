...
/**
 ...
 @static
 @param {String} filename file the WSDL collection is written to.  If not aupplied, the WSDL 
...
function makeWsdlCollection(filename, wsdlUrl, username, password, cb) {
    ...
    function checkForUrls() {
        ...
        var collectionString;
            try {
                collectionString = makeCollectionFile(filename, collection);
            }
            catch (err) {
                return cb(err);
            }
            cb(null, collectionString);
        }
    }
}
...
function makeCollectionFile(filename, collection) {
    ...
    result.push("</collection>");
    collection = result.join("\n");
    if (filename) {
        fs.writeFileSync(filename, collection);
        return;
    }
    else {
        return collection;
