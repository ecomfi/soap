...
function serialize(data, descriptor) {
    var doc = new soapdoc.SOAPDocument(namespaces.SOAPENV);
    ...
    descriptor.parts.forEach(function(part) {
        var jsName = part.jsName ? part.jsName : part.name;
        var value = data[jsName];
        var elmNs;
        var elmName;
    ...
    if (jsonType) {
      if (isArray) {
          ...
          data.forEach(function(row) {
            if (type) {
              doc.createLeafElement(xmlName, (type.xmlNs || xmlNs), formatJsonString(jsonType, data), isEncoded ? getXmlType(jsonType) : null);
            } else {
        ...
        if (type && type.nsUri) {
          doc.createLeafElement(xmlName, (type.xmlNs || xmlNs), formatJsonString(jsonType, data), isEncoded ? getXmlType(jsonType) : null, type.nsUri);
        } else {
          doc.createLeafElement(xmlName, xmlNs, formatJsonString(jsonType, data), isEncoded ? getXmlType(jsonType) : null);
        }
      }
        ...
        if (isArray) {
            ...
            if (isEncoded) {
                ...
                doc.addTypeAttr((type.arrayType || "array"), namespaces.SOAPENC_NS);
                data.forEach(function(row) {
                    serializeObject(doc, row, null, type, typeNs, (type.typeName || typeName), xmlNs, (type.xmlName || xmlName), isEncoded, false);
                });
                doc.endElement();
            }
            else { // not encoded
...
function serializeFields(doc, data, type, isEncoded) {
    ...
    type.content.forEach(function(item) {
        var jsName = item.jsName ? item.jsName : item.name;
        var value = data[jsName];
        if (value) {
            if (item.isAttr) {
