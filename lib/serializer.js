...
function serialize(data, descriptor) {
    var doc = soapdoc.init(descriptor.soapVersion == "1.1" ? namespaces.SOAP11_NS : namespaces.SOAP12_NS);
    ...
    if (jsonType) {
        ...
        if (isArray) {
            ...
            if (isEncoded) {
                ...
                doc.addAttr("arrayType", doc.ns.getPrefix(typeNs + ":" + typeName + "[" + data.length +  "]"), namespaces.SOAPENC_NS);
...
function serializeFields(doc, data, type, isEncoded) {
    if (type.baseType) {
        serializeFields(doc, data, baseType, isEncoded);
    }
