 ...
 **/
function createElement(name, nsuri, prefix) {
    var elm = new Element(this, name, nsuri, prefix);
...
/**
 ...
 @class Element
 **/
function Element(doc, name, nsuri, prefix) {
  console.log (name, nsuri, prefix)
    this.attrs = new Array();
    this.children = new Array();
    this.local = name;
    this.owner = doc;
    if (prefix) { this.pfx = prefix; }
    if (nsuri)  {
...
function serializeNs(pfx, ns, elmStrings) {
    if (pfx.indexOf('encodingStyle') === 0) {
      var nsDef = ' SOAP-ENV:' + pfx + '="' + escapeAttr(ns) + '"';
    }  else {
